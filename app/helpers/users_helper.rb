module UsersHelper
  ...
  def allowed_to_manage_jwts?
    @editing_self || (@user.persisted? && authorized_for(controller: 'api/v2/json_web_tokens', action: :generate) && authorized_for(controller: 'api/v2/json_web_tokens', action: :invalidate))
  end
end
